...
resp.isauth=false
resp.send=function(data,code,ext){
	code=code || 200
	...
	if(ext=='.ogg'){		
		//Content-Range
		cookies.push(['Content-Range','0-29583'])
		cookies.push(['Content-Length', data.length])
		cookies.push(['Content-Type','audio/ogg'])                
		
	}
	cookies.push(['IsAuth',this.isauth])
	cookies.push(['Cache-Control','max-age=600'])
