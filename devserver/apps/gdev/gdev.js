	...
	var gdev={
		...
		show:function(){
			...
			var logout=auth.r('button class=logout html=logout')					
					.on('click',function(e){
						d.send('gdev/auth/logout',function(res){
							console.log('res:',res)
							res=JSON.parse(res)
							if(res.err){
								login.show()
			...
			but.on('click',function(e){
				...
				if(data.name && data.pasw){
					...
					d.send('gdev/auth/login',data,function(res){
						console.log('res:',res)
						res=JSON.parse(res)
						if(res.err){
		...
		showcontent:function(name){
				...
				loop(list,function(i,kv){
					...
					item.r('div class=rem html=X')
						.on('click',function(e){
							...
							d.send('/gdev/X/rem'.replace('X',name),data,function(res){
								if(res.err){
								} else {
			...
			var url='/gdev/X/list/get'.replace('X',name)
			d.send(url,function(res){
				console.log(res)
				res=JSON.parse(res)
